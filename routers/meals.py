...
from lib.checks.checks import meal_exists, meal_type_exists
...
async def get_meal_by_date_and_meal_type(meal_type_id: int, date: date = None, token: str = Depends(oauth2_scheme)):
    _ = await get_current_user("admin", token=token)
    if not await meal_type_exists(meal_type_id):
        raise MealTypeNotFound()
...
async def update_meal(meal_id: int, meal: MealUpdate, token: str = Depends(oauth2_scheme)):
    _ = await get_current_user("admin", token=token)
