...
class DataBaseManager(metaclass=Singleton):
    ...
    async def update_major(self, major_id: int, major: MajorUpdate) -> None:
        query = (
            self.models.majors.update()
            .where(self.models.majors.c.id == major_id)
            .values(**major.dict())
        )
    ...
    async def get_student(self, id_or_email: int | str) -> Student | None:
        if isinstance(id_or_email, int):
            query = self.models.students.select().where(
                self.models.students.c.id == id_or_email
            )
        else:
            query = self.models.students.select().where(
                self.models.students.c.email == id_or_email
            )
        return await self.db.fetch_one(query)

    async def get_students_by_major(self, major_id: int) -> Page[Student]:
        query = self.models.students.select().where(
            self.models.students.c.major_id == major_id
        )
    ...
    async def update_student(self, student_id: int, **fields) -> None:
        query = (
            self.models.students.update()
            .where(self.models.students.c.id == student_id)
            .values(**fields)
        )
        await self.db.execute(query)

    async def delete_student(self, student_id: int) -> None:
        query = self.models.students.delete().where(
            self.models.students.c.id == student_id
        )
        await self.db.execute(query)

    async def get_meal_type_by_id(self, meal_type_id: int) -> MealTypeOut:
        query = self.models.meal_types.select().where(
            self.models.meal_types.c.id == meal_type_id
        )
        return await self.db.fetch_one(query)
    ...
    async def update_meal_type(
        self, meal_type_id: int, meal_type: MealTypeIn
    ) -> MealTypeOut:
        query = (
            self.models.meal_types.update()
            .where(self.models.meal_types.c.id == meal_type_id)
            .values(**meal_type.dict())
        )
        return await self.db.execute(query)

    async def delete_meal_type(self, meal_type_id: int) -> None:
        query = self.models.meal_types.delete().where(
            self.models.meal_types.c.id == meal_type_id
        )
        await self.db.execute(query)
    ...
    async def get_meal(self, meal_id: int) -> MealOut:
        query = (
            select(
                [
                    self.models.meals,
                    func.count(self.models.meal_students.c.student_id).label(
                        "students_count"
                    ),
                ]
            )
            .select_from(
                self.models.meals.outerjoin(
                    self.models.meal_students,
                    self.models.meals.c.id == self.models.meal_students.c.meal_id,
                )
            )
            .group_by(self.models.meals.c.id)
            .where(self.models.meals.c.id == meal_id)
        )
        return await self.db.fetch_one(query)
    ...
    async def get_all_meals(self) -> Page[MealOut]:
        query = (
            ...
            .group_by(self.models.meals.c.id)
            .order_by(self.models.meals.c.date_time.desc())
        )
    ...
    async def get_meal_student(self, meal_student_id: int) -> MealStudentOut:
        query = self.models.meal_students.select().where(
            self.models.meal_students.c.id == meal_student_id
        )
        return await self.db.fetch_one(query)

    async def get_all_meal_students(self, meal_id: int) -> Page[MealStudentOut]:
        query = self.models.meal_students.select().where(
            self.models.meal_students.c.meal_id == meal_id
        )
        return await paginate(self.db, query)

    async def get_student_meal(self, student_id: int, meal_id: int) -> MealStudentOut:
        query = self.models.meal_students.select().where(
            self.models.meal_students.c.student_id == student_id,
            self.models.meal_students.c.meal_id == meal_id,
        )
        return await self.db.fetch_one(query)

    async def update_meal_student(
        self, meal_student_id: int, meal_student: MealStudentIn
    ) -> None:
        query = (
            self.models.meal_students.update()
            .where(self.models.meal_students.c.id == meal_student_id)
            .values(**meal_student.dict())
        )
        await self.db.execute(query)

    async def delete_meal_student(self, meal_student_id: int) -> None:
        query = self.models.meal_students.delete().where(
            self.models.meal_students.c.id == meal_student_id
        )
        await self.db.execute(query)

    async def get_meal_by_date_and_meal_type(
        self, meal_type_id: int, date: str = None
    ) -> MealOut:
        if not date:
            query = (
                select(
                ...
                .group_by(self.models.meals.c.id)
                .where(
                    self.models.meals.c.meal_type_id == meal_type_id,
                    func.DATE(self.models.meals.c.date_time) == func.current_date(),
                )
            )
        else:
            query = (
                ...
                .where(
                    self.models.meals.c.meal_type_id == meal_type_id,
                    func.DATE(self.models.meals.c.date_time) == func.strftime(date),
                )
            )
    ...
    async def get_meals_by_date(self, date: str = None) -> Page[MealOut]:
        if not date:
            query = (
                ...
                .group_by(self.models.meals.c.id)
                .where(func.DATE(self.models.meals.c.date_time) == func.current_date())
            )
        else:
            query = (
                select(
                ...
                .group_by(self.models.meals.c.id)
                .where(func.DATE(self.models.meals.c.date_time) == func.strftime(date))
            )
    ...
    async def get_meals_by_meal_type(self, meal_type_id: int) -> Page[MealOut]:
        query = (
            ...
            .group_by(self.models.meals.c.id)
            .where(self.models.meals.c.meal_type_id == meal_type_id)
            .order_by(self.models.meals.c.date_time.desc())
        )
        return await paginate(self.db, query)
    ...
    async def get_admin(self, id_or_email: int | str) -> Admin | None:
        if isinstance(id_or_email, int):
            query = self.models.admins.select().where(
                self.models.admins.c.id == id_or_email
            )
        else:
            query = self.models.admins.select().where(
                self.models.admins.c.email == id_or_email
            )
        return await self.db.fetch_one(query)
    ...
    async def update_admin(self, admin_id: int, **fields) -> None:
        query = (
            self.models.admins.update()
            .where(self.models.admins.c.id == admin_id)
            .values(**fields)
        )
        await self.db.execute(query)
    ...
    async def get_student_absence_by_date(
        self, student_id: int, date: str = None
    ) -> Absence:
        query = f"""
        ...
        return await self.db.fetch_one(
            query=query, values={"student_id": student_id, "date": date}
        )

    async def get_absence(self, absence_id: int) -> Absence:
        query = self.models.absences.select().where(
            self.models.absences.c.id == absence_id
        )
        return await self.db.fetch_one(query)
    ...
    async def delete_absence(self, absence_id: int) -> None:
        query = self.models.absences.delete().where(
            self.models.absences.c.id == absence_id
        )
        return await self.db.execute(query)
